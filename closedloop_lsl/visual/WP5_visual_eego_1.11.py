...
sham_sound = op.join(cfg['DEFAULT']['SoundsPath'], 'sham_stim_50ms.wav')
...
trig_codes = {'stim': {'cingulate-lh': 22, 'cingulate-rh': 24},
              'sham': {'cingulate-lh': 32, 'cingulate-rh': 34}}
...
while True:
    ...
    if streamMouse.isPressedIn(startButton):
        ...
        detectWin = visual.Window(size=[500, 500], pos=(150, 150), 
                                  monitor=my_monitor, fullscr=False, 
                                  color='black', allowGUI=True, 
                                  title='SW selection')
        detectText = visual.TextStim(detectWin, text='Automatic SW Detection', 
                                     pos=(0, .8), height=0.12, color='cyan', 
                                     font='Night Sky', autoDraw=True)
        detectSubText = visual.TextStim(detectWin, text=('Click on \'Start '
                                        'detect\' to begin.'), pos=(0, .55), 
                                        height=0.065, color='white', 
                                        font='Nimbus Sans', autoDraw=True)
        startDetectText = visual.TextStim(detectWin, 
                                          text='Starting the detection...', 
                                          pos=(0, -.3), height=0.08, 
                                          color='white', font='Nimbus Sans')
        
        # Sel Stim or Sham
        selected_stim = 'stim'
        # This conditions are fixed for WP5
        selected_topo = 'cingulate'
        selected_peak = 'positive'
        
        # Topo selection buttons
        # selected_topo = 'cingulate'
        stimText = visual.TextStim(detectWin, text='Select the stimulation:', 
                                   pos=(0, .35), height=0.08, color='white', 
                                   font='Nimbus Sans', autoDraw=True)
        stim1Label = visual.TextStim(detectWin, text='Stim', 
                                     pos=(-.5, .1), height=0.075, 
                                     font='Nimbus Sans', autoDraw=True)
        stim2Label = visual.TextStim(detectWin, text='Sham', 
                                     pos=(.5, .1), height=0.075, 
                                     font='Nimbus Sans', autoDraw=True)
        stim1Button = visual.Circle(detectWin, radius=.05, pos=(-.5, .001), 
                                    fillColor='green', autoDraw=True)
        stim2Button = visual.Circle(detectWin, radius=.05, pos=(.5, .001), 
                                    fillColor='white', autoDraw=True)
        ...
        detectButton = visual.Rect(detectWin, width=.5, height=.2, 
                                   pos=(-.4, -.5), fillColor='green', 
                                   autoDraw=True)
        detectButtonLabel = visual.TextStim(detectWin, text='Start detect', 
                                            pos=(-.4, -.5), height=0.08, 
                                            font='Nimbus Sans', autoDraw=True)
        
        # Stop detection button
        stopDetectButton = visual.Rect(detectWin, width=.5, height=.2, 
                                       pos=(.4, -.5), fillColor='red', 
                                       autoDraw=True)
        stopDetectButtonLabel = visual.TextStim(detectWin, text='Exit', 
                                                pos=(.4, -.5), height=0.08, 
                                                font='Nimbus Sans', 
                                                autoDraw=True)
        ...
        while True:
            # Choose the stim type
            if detectMouse.isPressedIn(stim1Button):
                while detectMouse.getPressed()[0]:  # Wait until the mouse button is released
                    core.wait(0.0001)
                selected_stim = 'stim'
                stim1Button.fillColor = 'green'
                stim2Button.fillColor = 'white'
            elif detectMouse.isPressedIn(stim2Button):
                while detectMouse.getPressed()[0]:  # Wait until the mouse button is released
                    core.wait(0.0001)
                selected_stim = 'sham'
                stim1Button.fillColor = 'white'
                stim2Button.fillColor = 'green'
            ...
            if detectMouse.isPressedIn(detectButton):
                ...
                # Selecting the right stim type
                if selected_stim == 'stim':
                    st_type = 'stim'
                    used_trig = trig_codes['stim']
                    used_sound = stim_sound
                elif selected_stim == 'sham':
                    st_type = 'sham'
                    used_trig = trig_codes['sham']
                    used_sound = sham_sound
                else:
                    raise ValueError('Stimulation type not recognized.')
                                    
                # Selcting templates
                if selected_peak == 'positive':
                    peak = 'pos'
                else:
                    raise ValueError('Peak type not recognized.')
                # The used template is hardcoded 
                ...
                stimulator = Stimulator(stim_file=used_sound, 
                                        alarm_file=alarm_sound,
                                        trig_codes=used_trig)
                ...
                while not_really_sure_about_the_devices:
                    ...
                    if devsel.OK:
                        ...
                        print('''No way, you must select the devices. 
                              Make a wise choice.''')
                ...
                interruptWin = visual.Window(size=[300, 300], pos=(250, 250), 
                                             monitor=my_monitor, fullscr=False,
                                             color='black', allowGUI=True, 
                                             title='Be ready!')
                interruptText = visual.TextStim(interruptWin, 
                                                text=('Button will become '
                                                'green when a SW is detected.'
                                                '\nPress it within 5 seconds ' 
                                                'to stop stimulation.'), 
                                                pos=(0, .65), height=0.12, 
                                                color='white', wrapWidth=1.5, 
                                                font='Nimbus Sans', 
                                                autoDraw=True)
                interruptButton = visual.Rect(interruptWin, width=1.85, 
                                              height=1.2, pos=(0, -.35), 
                                              fillColor='gray', autoDraw=True)
                interruptButtonLabel = visual.TextStim(interruptWin, 
                                                       text=('Click to stop '
                                                       'stimulation'), 
                                                       pos=(0, -.35), 
                                                       height=0.28, 
                ...
                while running:
                    ...
                    sw_catcher.set_data(data.copy().drop_sel({'channels': 
                        ['TRIG1']}).values)
