...
while True:
    ...
    if streamMouse.isPressedIn(startButton):
        ...
        detectButton = visual.Rect(detectWin, width=.5, height=.2, pos=(-.4, -.76), fillColor='green', autoDraw=True)
        detectButtonLabel = visual.TextStim(detectWin, text='Start detect', pos=(-.4, -.76), height=0.08, font='Nimbus Sans', autoDraw=True)
        
        # Stop detection button
        stopDetectButton = visual.Rect(detectWin, width=.5, height=.2, pos=(.4, -.76), fillColor='red', autoDraw=True)
        stopDetectButtonLabel = visual.TextStim(detectWin, text='Exit', pos=(.4, -.76), height=0.08, font='Nimbus Sans', autoDraw=True)
        
        # Wake the subject button
        wakeButton = visual.Rect(detectWin, width=.3, height=.10, pos=(0., -.95), fillColor='orange', autoDraw=True)
        wakeButtonLabel = visual.TextStim(detectWin, text='Wake!', pos=(0., -.95), height=0.08, font='Nimbus Sans', autoDraw=True)
        ...
        while True:
            ...
            # Wake the subject up, and start the questonnaire
            elif detectMouse.isPressedIn(wakeButton):
                if running == False:
                    # Initialize the stimulator
                    ...
                    wake_stim = ['wake', False, 0, 0, 0]
                    
                    stimulator.send_stim(wake_stim)
                    
                    detectButton.fillColor = 'gray'
                    stopDetectButton.fillColor = 'gray'
                    wakeButton.fillColor = 'gray'
                    startDetectText.text = 'Waking up the participant...'
                    startDetectText.draw()
                    detectWin.flip()
                    ...
                    stopDetectButton.fillColor = 'red'
                    wakeButton.fillColor = 'orange'
                    startDetectText.text = 'Ready to detect'
                    startDetectText.draw()
                    detectWin.flip()
                    break
